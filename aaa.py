...
def _single_creative_score(
    ...
    content_tokens = _creative_content_tokens(
        creative_name
    )
    ...
    for segment in re.split(
        ...
        segment_normalized = _normalize(
            segment
        )

        if (
            len(segment_normalized) >= 7
            and segment_normalized
            in placement_normalized
        ):
            score += 20
    ...
    for record in records:
        ...
        if (
            ...
            if tied:
                ...
                warnings.append(
                    f"Ambiguous single-creative match for "
                    f"{placement_name}: "
                    + ", ".join(tied)
                )
...
def _version_url_for_creative(
    ...
    separate = _clean(
        separate_base_url_by_version.get(
            key,
            "",
        )
    )
    ...
    for index, record in enumerate(
        ...
        if (
            ...
            if not matches:
                ...
                warnings.append(
                    f"No Multi creative matched dimension "
                    f"{dimension}: "
                    f"{placement_name}"
                )
...
def _populate_multi_sheet(
    ...
    for record in records:
        ...
        for creative_name in matches:
            ...
            rotation = (
                _version_rotation_for_creative(
                    creative_name,
                    rotation_by_version,
                )
            )
